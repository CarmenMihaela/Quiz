...
function buildGameLayout() {
    ...
    function fillInValues() {
        ...
        let questions = []
        let questionIndex = 1
        let numberOfQuestions = makeRandomNumber(3)
        ...
        for (i = 0; i < numberOfQuestions; i++) {
            ...
            while (numbers.length < 2) {
                let number = makeRandomNumber(100)
                if (numbers.indexOf(number) === -1) numbers.push(number) //these are the other answers
            }

    
        let firstNumber = numbers[0]
            let operation = operations[makeRandomNumber(operations.length)]
            ...
            while (answers.length < 4) {
                let answer = makeRandomNumber(200)
